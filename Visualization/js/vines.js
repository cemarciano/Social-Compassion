...
function createVines(){
   ...
   // Draw vine
   function drawVinesWithLattice(lattice, seeds, interations, sort, prune, minLength, maxLength) {
		   ...
		   for (var i in branches) {
			   ...
			   if (branches[i].leaf) {
				   if (branches[i].active) {
					   ...
					   var x2 = ax * Math.pow(t + 0.1, 3) + bx * Math.pow(t + 0.1, 2) + cx * (t + 0.1) + dx;
					   var y2 = ay * Math.pow(t + 0.1, 3) + by * Math.pow(t + 0.1, 2) + cy * (t + 0.1) + dy;
